        ...
        if out_degrees:
            ...
            for i in range(6):
                if ik_angles is None:
                    ...
                    ik_angles = self.ik_solver.inverse(cords, False, q_guess=self.read_joints_pos(degrees=False))
                    if i == 5:
                        print("solve_ik could not find inverse solve after 3 attmpts (line 427)")
                        return None
            return self.convert_angles(
                ik_angles, to_degrees=True
            )
